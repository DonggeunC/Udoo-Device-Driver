...
#include "kerneltimer_dev_cdg.h"

#define   LEDKEY_DEV_NAME            "keyled_dev_cdg"
...
irqreturn_t sw_isr(int irq, void* dev_id){
	int i;
//	KERNEL_TIMER_MANAGER* pKtm = (KERNEL_TIMER_MANAGER*)dev_id;
	sw_no = 0;
...
static int ledkey_open (struct inode *inode, struct file *filp)
{
	...
	memset(pKtm,0x00,sizeof(KERNEL_TIMER_MANAGER));
	filp->private_data = (void*)pKtm;
	kerneltimer_registertimer(pKtm,pKtm->time_val);
	irq_init(filp);
	return 0;
}
...
static ssize_t ledkey_write (struct file *filp, const char *buf, size_t count, loff_t *f_pos)
{
	...
	int ret;
	//	get_user(kbuf,buf);
	ret = copy_from_user(&pKtm->led,buf,count);
	led_write(pKtm->led);
	return count;
	//	return -EFAULT;
}
